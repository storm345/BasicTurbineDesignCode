function [pol,foil] = xfoil(coord,debugName,alpha,Re,Mach,varargin)
...
% default filenames
wd = pwd;%fileparts(which(mfilename)); % working directory, where xfoil.exe needs to be
fname = mfilename;
file_coord= [wd foil_name '.foil'];
% Save coordinates
...
%   cmdFile = fopen('xfoilCmd.bat','w');
%   fwrite( cmdFile, cmd);
%   fclose(cmdFile);
%   [status,result] = system(sprintf('cd %s && start /min /wait xFoilCmd.bat',wd));
  [status,result] = system(cmd);
  if (status~=0),
    disp(result);
    disp("FOIL THAT FAILED: "+debugName);
